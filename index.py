...
def profile1():
    if request.method == "POST":
        ...
        # Add basic profile data to Firestore
        ...
        video_blob = bucket.blob(f"videos/{user_id}.mp4")
        video_blob.upload_from_file(video)
        video_blob.make_public()
        video_url = video_blob.public_url

        # Update Firestore profile with video URL
        db.collection("profile").document(user_id).update({"video_url": video_url})

        # Upload profile photo to Cloud Storage
        profile_photo = request.files["profile_photo"]
        photo_blob = bucket.blob(f"images/{user_id}.png")
        photo_blob.upload_from_file(profile_photo)
        photo_blob.make_public()
        photo_url = photo_blob.public_url

        # Update Firestore profile with profile photo URL
        db.collection("profile").document(user_id).update({"img": photo_url})
        ...
        flash("Profile and video uploaded successfully", category="success")
